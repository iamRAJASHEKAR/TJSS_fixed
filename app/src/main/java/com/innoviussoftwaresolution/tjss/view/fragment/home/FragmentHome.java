...
import com.innoviussoftwaresolution.tjss.model.network.response.UnreadMessage;
import com.innoviussoftwaresolution.tjss.model.network.response.nearby.Geometry;
import com.innoviussoftwaresolution.tjss.model.network.response.nearby.NearbySearchResponse;
import com.innoviussoftwaresolution.tjss.model.network.response.nearby.Result;
...
import com.innoviussoftwaresolution.tjss.utils.extensions.BitmapToPinExtension;
import com.innoviussoftwaresolution.tjss.view.activity.AppServiceProviderListingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.ChatDetailActivity;
import com.innoviussoftwaresolution.tjss.view.activity.CoinPurchaseActivity;
import com.innoviussoftwaresolution.tjss.view.activity.CreateCircleActivity;
import com.innoviussoftwaresolution.tjss.view.activity.FaqActivity;
import com.innoviussoftwaresolution.tjss.view.activity.FavPlacesActivity;
import com.innoviussoftwaresolution.tjss.view.activity.HelpAlertActivity;
import com.innoviussoftwaresolution.tjss.view.activity.HelpAlertHistoryActivity;
import com.innoviussoftwaresolution.tjss.view.activity.IncidentsListingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.InvoiceActivity;
import com.innoviussoftwaresolution.tjss.view.activity.JoinCircleActivity;
import com.innoviussoftwaresolution.tjss.view.activity.LocationSharingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.MessagesActivity;
import com.innoviussoftwaresolution.tjss.view.activity.PlansListingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.PurchaseServiceActivity;
import com.innoviussoftwaresolution.tjss.view.activity.SafetyCircleMembersListingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.SafetyCircleMembersListingForCheckInActivity;
import com.innoviussoftwaresolution.tjss.view.activity.SafetyTipsListingActivity;
import com.innoviussoftwaresolution.tjss.view.activity.SecurityQuestionsActivity;
...
import com.innoviussoftwaresolution.tjss.view.activity.UserProfileActivity;
...
import com.innoviussoftwaresolution.tjss.view.fragment.generalalerts.ProgressDialog;
...
import com.innoviussoftwaresolution.tjss.viewmodel.UserHomeViewModel;
...
public class
FragmentHome extends BaseFragment implements OnMapReadyCallback,
    ...
    TabLayout.Tab safetyCircle;
    TabLayout.Tab favoritePlaces;
    TabLayout.Tab incidents;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == android.R.id.home) {
            ...
            try {
                final boolean locationShare = PreferenceHelper.getBoolean(getActivity(), PreferenceHelper.KEY_LOCATION_SHARED, true);
...
//                    jsonObject.put("locName", place.getAddress().toString().split(",")[1]);
...
//                            .sendBroadcast(intent);
                    ...
                    checkInModel.setPlaceId(PreferenceHelper.getString(getActivity(), PreferenceHelper.KEY_USER_ID, ""));
                    checkInModel.setLat(String.valueOf(place.getLatLng().latitude));
                    checkInModel.setLongt(String.valueOf(place.getLatLng().longitude));
                    checkInModel.setLocName(place.getName().toString());
                    //checkInModel.setImage(image);
                    addCheckin(checkInModel);
            ...
            // M.showSnackBar(getActivity(),"Checked in failed");
        }
    }

    //Request location sharing with circle members
    public void locationSharing() {

        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setTitle("Location Sharing");
        builder.setMessage("You are not sharing your location with circle. Enable location sharing to check in.");
        builder.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {

                Intent intent = new Intent(getActivity(), LocationSharingActivity.class);
    ...
    //Add check in histroy in database
    public void addCheckin(final CheckInModel checkInModel) {
        EndpointsServices services = APIClient.getClient().create(EndpointsServices.class);
        Call<CheckInModel> call = services.addCheckInHistory(checkInModel);
        call.enqueue(new Callback<CheckInModel>() {
            ...
            public void onResponse(Call<CheckInModel> call, Response<CheckInModel> response) {
                if (response != null) {
                    if (response.body() != null) {
                        ...
                        //Log.e("Response Check in==>", new GsonBuilder().setPrettyPrinting().create().toJson(response));
                        Toast.makeText(getActivity(), "Checked in successfully", LENGTH_LONG).show();
                    }
                } else {
                    makeText(getActivity(), "Null response", LENGTH_LONG).show();
                }
            }
            ...
            public void onFailure(Call<CheckInModel> call, Throwable t) {
                ...
                makeText(getActivity(), "Failed to add check in", LENGTH_LONG).show();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        safetyCircle = mHomeTabs.getTabAt(0);
        favoritePlaces = mHomeTabs.getTabAt(1);
        incidents = mHomeTabs.getTabAt(2);
...
//        SharedPreferences.Editor editor=preferences.edit();
        ...
        // String profileImage = getString(R.string.image_base_url) + "Jellyfish1.jpg";
    ...
    private void bindMarkers(@DrawableRes int markerIcon, @NonNull NearbySearchResponse response) {
        ...
        for (Result result : results) {
            ...
            com.innoviussoftwaresolution.tjss.model.network.response.nearby.Location location = geometry.location;
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        //  Toast.makeText(getContext(),PreferenceHelper.getString(getContext(),PreferenceHelper.KEY_USER_ID,""),Toast.LENGTH_LONG).show();
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.btnDrawerHelpAlert:
                ...
                if (hasSecurity) {
                    boolean permission = PreferenceHelper.getBoolean(getActivity(), PreferenceHelper.KEY_LOCATION_SHARED, true);
                    if (permission == true) {
            ...
            case R.id.btnDrawerServiceProviders:
                ...
                if (hasSecurityAdded)
                    ...
                    makeText(getActivity(), "Please select your security question to continue", LENGTH_SHORT).show();
                    startActivity(new Intent(getActivity(), SecurityQuestionsActivity.class));
                }
                break;
        ...
        switch (selectedTab) {
            ...
            case INCIDENTS: {
                ...
                try {
                    ...
                    Bugsnag.notify(new RuntimeException(e));
                    M.log(e.getMessage());
                    makeText(getActivity(), e.getMessage(), LENGTH_SHORT).show();
                }
...
/*
                    if(selectedTab==FAV_PLACES)
                    {

                    }
                  //  Toast.makeText(getContext().getApplicationContext(), "good Morning", Toast.LENGTH_SHORT).show();
...
*/
    ...
    private class FavPlaceDialogRunnable implements Runnable {
        ...
        @Override
        public void run() {
            FavPlaceMarkerInfoWindowDialog dialog = FavPlaceMarkerInfoWindowDialog.newInstance(mParts);
            dialog.show(getChildFragmentManager(), "favPlaceDialog");
            if (!dialog.isVisible())
                mGoogleMap.clear();
            dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                @Override
...
//                    mViewModel.getSafetyCircles();*/
    ...
    private class IncidentsDialogRunnable implements Runnable {
        ...
        @Override
        public void run() {
            IncidentMarkerInfoWindowDialog dialog = IncidentMarkerInfoWindowDialog.newInstance(mParts);
            dialog.show(getChildFragmentManager(), "favPlaceDialog");
            if (!dialog.isVisible())
                mGoogleMap.clear();
            dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
