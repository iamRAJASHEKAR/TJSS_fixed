...
import com.innoviussoftwaresolution.tjss.view.activity.SignUpActivity;
import com.innoviussoftwaresolution.tjss.view.adapter.pageradapter.SignInPagerAdapter;
import com.innoviussoftwaresolution.tjss.view.fragment.signup.LoginSignUpFragmentBase;
import com.innoviussoftwaresolution.tjss.viewmodel.LoginViewModel;
...

{
    ...
    public void onMBtnNextClicked()
    {
        ...
        SharedPreferences preferences =getActivity().getSharedPreferences("firebaseid",0) ;
        SharedPreferences.Editor editor=preferences.edit();

        String firebaseId=  UUID.randomUUID().toString();
        //editor.putString("firebaseId",firebaseId);
        PreferenceHelper.saveString(getActivity(),"firebaseId",firebaseId);
        editor.commit();
        ...
        else if (mVpSignIn.getCurrentItem() == mVpSignIn.getAdapter().getCount() - 1)
        {
            ...
            if (index < 0)
            {
                ...
                mViewModel.newLogin(phone,password,firebaseId);
            } else {
